...
const SearchInput: React.FC<SearchInputProps> = ({ onSearch, isLoading, language, isSeniorMode = false }) => {
  ...
  const loadImageFile = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = async () => {
        ...
        // Run Tesseract OCR in browser — supports Traditional Chinese + English
        try {
  ...
  return (
    <div className={`w-full mx-auto mb-12 ${isSeniorMode ? 'max-w-3xl' : 'max-w-2xl'}`}>
      ...
      {image && (
        <div className="relative mb-3 rounded-2xl overflow-hidden border border-crypto-accent/40 bg-gray-900">
          ...
          <div className="flex items-center justify-center gap-2 py-2">
            {ocrStatus === 'running' && <Loader2 className="w-4 h-4 text-crypto-accent animate-spin" />}
            {ocrStatus !== 'running' && <ScanText className="w-4 h-4 text-crypto-accent" />}
      ...
      {/* OCR done notice — shown after image is cleared */}
        ...
        {useTextarea ? (
          <div className={`relative bg-gray-900 border border-gray-700 shadow-2xl overflow-hidden focus-within:border-crypto-accent transition-colors rounded-2xl`}>
            ...
            <textarea
              ...
              value={input}
              onChange={(e) => { setInput(e.target.value); if (txtFileName) setTxtFileName(null); if (ocrStatus === 'done') setOcrStatus('idle'); }}
              disabled={isLoading}
            />
